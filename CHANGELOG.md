...
### 2026-09-23 (15) — Pig mud-play now travels to the mud patch; Character screen: Tamers gallery + bulked perk summary

**Pig's mud-play chance was effectively far rarer than `PERK_CHANCES.pigMud` implies, and here's why.** The previous fix (entry 13) made mud-play only fire if the pig HAPPENED to already be standing inside the mud patch at the exact instant a random, unrelated food/water forage completed — but the pig's forage targets are scattered across the whole sty, and the patch is a small ellipse in the middle of it. The odds of "the pig happens to be there right now" turning out to be true are much lower than the 5-8% roll itself, so in practice mud-play became a rare coincidence rather than a genuine 1-in-20-ish event — which matches "only fired once in 3 minutes."

Fixed by giving the pig a real destination: a successful forage now rolls `pigMud` exactly as before, but on a win the pig sets off for the mud patch first (`'mud_travel'`, a new state that walks it to `getMudPatch()`'s center at 1.2x speed — the same `fishing_travel` → `fishing` pattern the bear already uses to walk to the lake) and only flips to `'mud_play'` once it's actually arrived and `isInMudPatch()` confirms it. The floating label now reads `[MUD PLAY]` for both legs (heading there and playing), matching how the bear's `fishing_travel` already reads as `[FISHING]`. Verified the full cycle end-to-end by running the pig's real `update()` in Node: walked from a corner to the patch in 103 ticks, `isInMudPatch()` true on arrival, played for ~5.5s, paid out coins, returned to `wander` — no dead ends, no stuck states.

**On "pigs were also sped up" — investigated, but couldn't find a bug, and I don't want to guess wrong here.** Mud-play itself never touches speed (confirmed again reading through it while making the above fix) — it's purely a 5-second visual/coin state. I also re-checked `_regionSpeedMult` (the only speed-multiplier system in the game): it's recomputed from scratch for every pet, every frame, straight from live state, so there's no path for a stale boost to "stick." The one thing that WOULD genuinely speed up a pig in Region 6 is the bird's Lv20+ excursion visiting there — Region 6 is one of the five regions in `BIRD_VISIT_SPEED_REGIONS` (entry 13, at your own request) — which requires the bird to already be Lv20+. If your save's bird is high-level, that's almost certainly what you saw, not a bug. I didn't remove Region 6 from that list since it's what was explicitly asked for last time — flagging this instead so you can tell me whether to take it back out.

**Character screen: model switch button → Tamers gallery.** Replaced the single "Switch to Boy/Girl" toggle with a `🎭 Tamers` section listing every `PLAYER_MODELS` entry (entities.js) as its own small card — live idle-pose preview, name, and either a green `✓ IN USE` badge (the equipped one, not clickable — there's nothing to do) or a `USE` button (every other one) that switches immediately, no reload. Built from `PLAYER_MODELS` itself rather than two hardcoded cards, so a third model added later just appears. `selectCharacterModel(model)` (ui.js) replaces the old toggle handler.

**Character screen: perk summary is now bulked by perk type.** The Perk Tree has several separate nodes of the same type (e.g. 6 different "Basic Resource" nodes across tiers/columns) — the Character screen's perk checklist used to print one identical-looking `+30%` line per node once several were unlocked, which just reads as clutter. It now groups by `PERK_TYPES` and shows ONE line per type with the SUMMED percentage from however many of that type are actually unlocked, plus an "(X/Y unlocked)" count so the granularity isn't lost — e.g. `✓ 🍪💧 Basic Resource: +90% food & water gained from pets (3/6 unlocked)`. Added a `desc` field to `PERK_TYPES` (state.js) for this — the number-free tail of what used to be baked into each node's own `text` — so the summed line isn't stuck repeating one node's fixed `+30%`. The Perk Tree screen itself (where you actually spend points) is untouched — each node there still shows its own individual effect.

**Verification:** `node --check` on every edited file. Loaded state.js+entities.js+world.js together in Node and: ran the pig's actual `update()` through a full `mud_travel → mud_play → payout → wander` cycle (shown above); computed the grouped-perk math directly against a simulated `character.perks` array and confirmed 3 unlocked Basic Resource nodes summed to +90%, and a type with zero unlocked nodes correctly showed 0; confirmed `PLAYER_MODELS` exposes both `female` and `male` for the Tamers list to iterate. `index.html`'s div tags stayed balanced after the markup swap. **Not tested:** an actual browser/Playwright run (unavailable in this session) — worth a look, especially the Tamers cards' layout on a small screen.
